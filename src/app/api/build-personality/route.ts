...
export async function POST(request: NextRequest) {
  ...
  const endpoint = `${A0X_AGENT_API_URL}/a0x-agent/build-personality`;
...
export async function DELETE(request: NextRequest) {
  console.log("[DELETE][api/build-personality]");

  const body = await request.json();

  const userAddress = body.userAddress?.toLowerCase();

  const endpoint = `${A0X_AGENT_API_URL}/a0x-agent/delete-memory-personality`;
